...
final case class JsonRpcServer[Json, Effect[_]](
  ...
  def process(request: ArraySeq[Byte]): Effect[ArraySeq[Byte]] = {
    ...
    effectContext.map(process(text), { response =>
      ArraySeq.unsafeWrapArray(response.getBytes(charset).nn)
    })
  }
  ...
  def process(request: InputStream): Effect[InputStream] =
    ...
    effectContext.map(process(text), { response =>
      new ByteArrayInputStream(response.getBytes(charset).nn)
    })
