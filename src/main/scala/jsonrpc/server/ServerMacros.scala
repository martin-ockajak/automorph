...
object ServerMacros:
  ...
  private def bind[T <: AnyRef: Type](api: Expr[T])(using quotes: Quotes): Expr[Unit] =
    ...
    val baseMethodNames =
      Seq(TypeTree.of[AnyRef], TypeTree.of[Product]).flatMap { typeTree =>
        ref.methods(typeTree).filter(_.public).map(_.name)
      }.toSet
    ...
    val apiMethods = ref.methods(apiTypeTree).filter(_.public).filter { method =>
      !baseMethodNames.contains(method.symbol.name)
    }
    apiMethods.filterNot(_.concrete).foreach { method =>
