...
import automorph.codec.json.JacksonJsonCodec.Node
...
//      }, {
        ...
        val yhandler: Handler[Node, codec.type, Effect, Context] = Handler.protocol[Node, codec.type, Context](protocol).system(system)
        val xhandler: Handler[Node, codec.type, Effect, Context] = yhandler.bind[SimpleApiType](simpleApi)
        val handler: Handler[Node, codec.type, Effect, Context] = xhandler.bind[ComplexApiType](complexApi)
