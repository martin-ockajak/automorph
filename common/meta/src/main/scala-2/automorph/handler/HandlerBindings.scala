...
case object HandlerBindings {
  ...
  def brokenGenerateMacro[
    ...
    val handlerBindings = validMethods.map { method =>
      q"${method.name} -> ${brokenGenerateBinding[c.type, Node, ActualFormat, Effect, Context, Api](ref)(method, format, system, api)}"
    }
  ...
  private def brokenGenerateBinding[
    C <: blackbox.Context,
    ...
    val invoke = brokenGenerateInvoke[C, Node, ActualFormat, Effect, Context, Api](ref)(method, format, system, api)
  ...
  private def brokenGenerateInvoke[
    C <: blackbox.Context,
    Node: ref.c.WeakTypeTag,
    ActualFormat <: MessageFormat[Node]: ref.c.WeakTypeTag,
    Effect[_],
